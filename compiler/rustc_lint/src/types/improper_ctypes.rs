...
impl<'a, 'tcx> ImproperCTypesVisitor<'a, 'tcx> {
    ...
    fn visit_fnptr(
        ...
        // // if this is not called from check_for_fnptr, then this enum is visited in the middle of another lint,
        ...
        // let override_cause_ty = if state != CTypesVisitorState::None { Some(ty) } else { None };
    ...
    fn visit_variant_fields(
        ...
        for (field_i, field) in field_list.into_iter().enumerate() {
            ...
            if let FfiUnsafe(ref reasons) = ffi_res {
                if let (1, Some(FfiUnsafeExplanation { ref reason, .. })) =
    ...
    fn visit_type(
        ...
        match *ty.kind() {
            ty::Adt(def, args) => {
                ...
                // FIXME: maybe dedup lints with def.did().is_local()
                match def.adt_kind() {
            ...
            ty::FnPtr(sig_tys, hdr) => {
                ...
                let inherent_safety = if fn_abi_is_internal(sig.abi()) {
                    FfiResult::new_with_reason(
                        ty,
                    ...
                    FfiSafe //self.visit_fnptr(state, outer_ty, ty, sig)
                };
            ...
            ty::Param(..) | ty::Alias(ty::Projection | ty::Inherent | ty::Weak, ..) => {
                if ty.has_opaque_types() {
                    ...
                    ) && state.can_expect_ty_params()
                    {
                        ...
                        // ty::Alias(ty::Weak), and all params/aliases for something
                        // defined beyond the FFI boundary
                        println!("{:?} {:?}", state, outer_ty);
                        match ty.kind() {
                            ty::Param(..) => println!("param!"),
                            ty::Alias(ty::Projection, ..) => println!("projection!"),
                            ty::Alias(ty::Inherent, ..) => println!("inherent!"),
                            ty::Alias(ty::Weak, ..) => println!("weak!"),
                            ty::Alias(ty::Opaque, ..) => println!("opaque!"),
                            _ => println!("fuck if I know"),
                        }
                        // FfiResult::new_with_reason(
                        //     ty,
                        //     fluent::lint_improper_ctypes_char_reason,
    ...
    fn check_for_adtdef(&mut self, ty: Ty<'tcx>) -> FfiResult<'tcx> {
        ...
        match &mut ffires {
            ...
            FfiUnsafe(ref mut explanations) => {
                explanations.iter_mut().for_each(|explanation| {
